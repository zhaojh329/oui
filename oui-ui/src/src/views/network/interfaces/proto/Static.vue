<template>
  <a-tabs>
    <a-tab-pane key="general" :tab="$t('General Settings')">
      ...
      <oui-form-item-input :uci-section="uciSection" :label="$t('IPv4 address')" name="ipaddr" required rules="ip4addr" @change="ipChange"/>
      <oui-form-item-select :uci-section="uciSection" :label="$t('IPv4 netmask')" name="netmask" allow-create @change="maskChange" :options="['255.255.255.0', '255.255.0.0', '255.0.0.0']" required rules="netmask4"/>
      <oui-form-item-input :uci-section="uciSection" :label="$t('IPv4 broadcast')" name="broadcast" rules="ip4addr" :placeholder="broadcast"/>
      <oui-form-item-input :uci-section="uciSection" :label="$t('IPv4 gateway')" name="gateway" rules="ip4addr"/>
      <oui-form-item-list :uci-section="uciSection" :label="$t('DNS servers')" name="dns" rules="ipaddr"/>
    </a-tab-pane>
