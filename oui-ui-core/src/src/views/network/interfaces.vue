  ...
  <div>
    <a-table :data-source="interfacesDataSource" :columns="columns">
      ...
      <template v-slot:status="record">
        <strong>{{ $t('Uptime') }}</strong>: {{ record.isUp() ? '%t'.format(record.getUptime()) : $t('Interface is down') }}<br/>
      ...
      <template v-slot:action="record">
        <a-button type="primary" size="small" style="margin-right: 5px" @click="ifup(record.name)">{{ $t('Start') }}</a-button>
        <a-button type="warning" size="small" style="margin-right: 5px" @click="ifdown(record.name)">{{ $t('Stop') }}</a-button>
        <a-button type="primary" size="small" style="margin-right: 5px" @click="edit(record.name)">{{ $t('Edit') }}</a-button>
        <a-popconfirm @confirm="del(record.name)" placement="left" :title="$t('Really delete this interface? The deletion cannot be undone!You might lose access to this device if you are connected via this interface.')">
          <a-button type="danger" size="small">{{ $t('Delete') }}</a-button>
        </a-popconfirm>
      </template>
    </a-table>
    <a-button type="primary" size="small" style="margin-top: 10px" @click="handleAdd">+ {{ $t('Add interface') }}</a-button>
...
export default {
  ...
  methods: {
    ...
    protoChanged (self) {
      ...
      this.$confirm({
        title: this.$t('Switch protocol'),
        content: this.$t('Are you sure you want to switch the protocol?'),
        onOk: () => {
    ...
    handleAdd () {
      this.$prompt({
        title: this.$t('Add interface'),
        placeholder: this.$t('Please input a name')
      }).then(name => {
        if (!name) return

        if (name.match(/^[a-zA-Z0-9_]+$/) === null) {
          this.$message.error(this.$t('Must be a valid UCI identifier'))
          return
        }
