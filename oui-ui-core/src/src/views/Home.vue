      ...
      <div style="clear: both">
        ...
        <div v-show="tab === 'router'">
          <div style="display: flex; justify-content: space-around;">
            <oui-dashboard :percent="cpuPercentage" :label="$t('CPU usage')"/>
            <oui-dashboard :percent="memPercentage" :label="$t('Memory usage')"/>
            <oui-dashboard :percent="storagePercentage" :label="$t('Storage')"/>
          </div>
          <a-descriptions :title="$t('System information')" :column="1" bordered>
  ...
  methods: {
    ...
    update () {
      this.$ubus.call('oui.system', 'cpu_time').then(({ times }) => {
        ...
        const idle1 = this.lastCPUTime[3] + this.lastCPUTime[4]
        const idle2 = times[3] + times[4]
