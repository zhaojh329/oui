<template>
   <uci-form config="system" v-if="sysfs.length > 0">
    <uci-section type="led" addable :teasers="['name', 'sysfs', 'default', 'trigger']">
      ...
      <uci-option type="input" :label="$t('On-State Delay')" name="delayon" required depend="trigger == 'timer'" :description="$t('Time in milliseconds the LED stays on')"></uci-option>
      <uci-option type="input" :label="$t('Off-State Delay')" name="delayoff" required depend="trigger == 'timer'" :description="$t('Time in milliseconds the LED stays off')"></uci-option>
