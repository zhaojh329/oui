...
uci.reorderSections = function () {
  ...
  */
  for (const conf in r) {
    ...
    if (sids.length > 0) { batch.push(['uci', 'order', { config: conf, sections: sids }]) }
  }
...
uci.save = function () {
  ...
  return new Promise(resolve => {
    ...
    for (const conf in c) {
      for (const sid in c[conf]) { batch.push(['uci', 'set', { config: conf, section: sid, values: c[conf][sid] }]) }
      confs[conf] = true
    }
    ...
    for (const conf in n) {
      for (const sid in n[conf]) {
        ...
        for (const k in n[conf][sid]) {
          if (k === '.type') { params.type = n[conf][sid][k] } else if (k === '.create') { params.name = n[conf][sid][k] } else if (k.charAt(0) !== '.') { params.values[k] = n[conf][sid][k] }
        }
...
uci.apply = function (timeout, rollback) {
  ...
  return new Promise((resolve, reject) => {
    ubus.call('uci', 'apply', { rollback, timeout }).then(() => {
      ...
      const tryDeadline = date.getTime() + 1000 * timeout
      const tryConfirm = function () {
        ubus.call('uci', 'confirm').then(() => {
          state.applying = false
          resolve()
        }).catch(e => {
          if (date.getTime() < tryDeadline) {
            window.setTimeout(tryConfirm, 250)
            return
          }
          state.applying = false
          reject(e)
      ...
      window.setTimeout(tryConfirm, 1000)
    }).catch(e => {
      state.applying = false
      reject(e)
    })
