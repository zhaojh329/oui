<template>
  <oui-form-item-template v-bind="OuiFormItemTemplateProps" ref="form-item" :rules="listRules">
    <div class="oui-form-list">
      <a-tag v-for="tag in model" :key="tag" :closable="!disabled" @close="handleDelDlist(tag)">{{ tag }}</a-tag>
      ...
      <a-button v-else size="small" type="primary" @click="showInput" style="width: 100px" :disabled="disabled">+ {{ $t('Add') }}</a-button>
    </div>
